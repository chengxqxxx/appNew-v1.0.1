...
<div class="main-content home-main-content">
  <div class="main-wrapper col-sm-12">
    <div class="main-card">
      ...
      <div class="connection-word" v-if='wancheng'>商品同步完毕，即将进入功能后台</div>
      <div class="connection-buttons" v-if='wancheng'>
        <button class="return-btn"><router-link to='/dashboard/home'>返回首页</router-link></button>
        <button class="start-btn"><router-link :to="'/goodsmanage/'+$route.params.shopid+'/home'">进入功能界面</router-link></button>
      </div>
    </div>
  </div>
</div>
...
export default {
  ...
  methods: {
      ...
      this.$Axios.post(this.$ApiUrl + 'productmanage/productinit', {
        ...
        if (response.data.code == 200) {
          ...
          if (begin != end) {
            ...
            this.text2 = '第（1/3）步，商品同步完成，即将进行通知管理设置！';
            this.tongzhistart();
          }
        } else {
          console.log('shiwu');
    ...
    tongzhistart() {
      this.tongzhi = true;
      this.text3 = '对接接口正在安装中。。。请勿关闭该窗口'
      this.tongzhiprocess(0, this.tongzhiitems);
    },
    tongzhiprocess(index, items) {
      this.$Axios.get(this.$ApiUrl + 'productmanage/webhookini/' + items[index].topic + '/' + this.$route.params.shopid)
        .then(response => {
          if (response.data.code == 200) {
    ...
    initfinish() {
      ...
      this.$Axios.post(this.$ApiUrl + 'productmanage/productinit3', {
        token: localStorage.getItem('token'),
        shopid: this.$route.params.shopid,
  ...
  created() {
    this.$Axios.get(this.$ApiUrl + 'productmanage/productinicheck?token=' + localStorage.getItem('token') + '&shopid=' + this.$route.params.shopid + '&functionid=' + this.$route.params.functionid)
      .then(response => {
        if (response.data.code == 200) {
          this.text1 = '店铺连线成功！一共有' + response.data.count + '个产品需要进行同步。。';
          this.tongbu = true;
